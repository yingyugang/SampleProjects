...
public class CameraDragZoom : MonoBehaviour {
    ...
    void Update()
    {
        ...
        {
            Vector3 pos = Camera.main.ScreenToViewportPoint(Input.mousePosition - mouseOrigin);
            if (transform.localPosition.x < -1.6f)
            {
                transform.localPosition = new Vector2(-1.6f, transform.localPosition.y);
            }
            if (transform.localPosition.x > 1.6f)
            {
                transform.localPosition = new Vector2(1.6f, transform.localPosition.y);
            ...
            {
                transform.localPosition = new Vector2(transform.localPosition.x, -1.1f);
            }
            if (transform.localPosition.y > 1.1f)
            {
                transform.localPosition = new Vector2(transform.localPosition.x, 1.1f);
